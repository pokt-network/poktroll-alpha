...
func (k msgServer) UnstakeServicer(goCtx context.Context, msg *types.MsgUnstakeServicer) (*types.MsgUnstakeServicerResponse, error) {
	...
	logger := k.Logger(ctx).With("method", "UnstakeServicer")
	...
	application, found := k.GetServicers(ctx, msg.Address)
	if !found {
		logger.Error(fmt.Sprintf("application not found for address %s", msg.Address))
		return nil, types.ErrUnstakingNonExistentServicer
	}
	...
	k.RemoveServicers(ctx, msg.Address)
